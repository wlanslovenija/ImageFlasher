...
{
  return (lwipcapif->flags & PCAP_IF_LOOPBACK) != 0 ;
}
...
struct sockaddr* get_addr(struct lwipcap_addr* lwipaddr)
{
  return lwipaddr->addr;
}

struct sockaddr* get_netmask(struct lwipcap_addr* lwipaddr)
{
  return lwipaddr->netmask;
}

struct sockaddr* get_broadcast(struct lwipcap_addr* lwipaddr)
{
...
{
  struct lwipcap* lwipcap = safe_malloc(sizeof(struct lwipcap));
