...
elif [ "$2" == "asuswl500gpv2" ]; then
	echo Flashing 192.168.1.1 using $1...
	echo -en "rexmt 1\ntrace\nbinary\nput $1\nquit\n" | tftp 192.168.1.1
	echo Please wait few minutes and then power cycle the router. Then you can login via telnet 192.168.1.1.
elif [ "$2" == "linksys" ]; then
	echo Flashing 192.168.1.1 using $1...
	echo -en "rexmt 1\ntrace\nbinary\nput $1\nquit\n" | tftp 192.168.1.1
	echo Please wait until power led stops flashing. Do not poweroff! Then you can login via telnet 192.168.1.1.
elif [ "$2" == "buffalo" ]; then
	echo Flashing 192.168.11.1 using $1...
	echo -en "rexmt 1\ntrace\nbinary\nput $1\nquit\n" | tftp 192.168.11.1
	echo Please wait until power led stops flashing. Do not poweroff! Then you can login via telnet 192.168.11.1.
elif [ "$2" == "toshiba" ]; then
